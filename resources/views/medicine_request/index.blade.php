...
<div class="card">
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            {!! Form::open(['route' => 'medicineRequest.index', 'method' => 'POST', 'class' => 'row row-cols-lg-auto g-3 align-items-center', 'autocomplete' => 'off']) !!}
            @csrf
            ...
            <div class="col-12">
                ...
                <button type="button" class="btn btn-info d-none" onclick="updateStatusChange()">Change status</button>
                {!! Form::close() !!}
            </div>
        </div>
    </div>
                    ...
                    <tr>
                        <!-- @php
                            $mids = \App\Models\MedicineRequest::select(DB::raw('GROUP_CONCAT(id) AS mrId'))
                                ->where(['arogyamitra_id' => $medicine['arogyamitra_id'], 'status'=> '1', 'medicine_id'=> $medicine['id']])
                        ...
                        @if (Auth::user()->role == 1)
                            @php
                                $stock = \App\Models\MedicineStock::select('qty')
                                    ->where(['arogyamitra_id' => Auth::user()->id, 'medicine_id'=> $medicine['id']])
                                    ->first();
                            @endphp
                            ...
                            <td class="text-center">
                                @if($medicine['status'] == 1)
                                    <div class="badge badge fw-bolder">
                                        <a href="javascript:void(0);" type="submit" class="btn btn-xs btn-success acceptStock" data-toggle="tooltip" data-bs-custom-class="tooltip-dark" title="Accept" data-medicine="{{ $medicine['id'] }}" data-arogyamitra_id="{{ $medicine['arogyamitra_id'] }}" ><i class="fa fa-check"></i></a>
                                        ...
                                        {!! Form::button('<i class="fa fa-ban"></i>', array('type'=>'submit','class' => 'btn btn-xs btn-danger','data-toggle'=>'tooltip', 'data-bs-custom-class'=>"tooltip-dark", 'title'=>'Reject')) !!}
                                        {!! Form::close() !!}
                                    </div>
