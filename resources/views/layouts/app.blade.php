                    ...
                    <div class="toolbar" id="kt_toolbar">
                        ...
                        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                            ...
                            @if (Request::segment(1) != 'home' &&
                                    Request::segment(1) != 'medicineRequest' &&
                                    Request::segment(1) != 'profile' &&
                                    Request::segment(1) != 'password' &&
                                    Request::segment(1) != 'medicine-stock' &&
                                    Request::segment(1) != 'report')
                                @if (!Request::is(Request::segment(1) . '/*'))
                                    @if (
                                        !in_array(Request::segment(1) . '/create', $routeArr) &&
                                            !in_array(Request::segment(1) . '/*', $routeArr) &&
                                            Request::segment(2) != 'create' &&
                                            !in_array(Request::segment(3), $routeArr))
                                        <div class="card-toolbar">
                                            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                                @if (Request::segment(1) == 'users')
                                                    <a href="{{ route('user-export') }}" class="btn btn-info me-5">Full
                                                        Export</a>
                                                @endif
                                                ...
                                                @if (!in_array(Request::segment(1), $noCreateModules) && Route::has(Request::segment(1) . '.create'))
                                                    @if (Auth::user()->role == '1' || Auth::user()->role == '4')
                                                        <a href="{{ route(Request::segment(1) . '.create') }}"
                                                            class="btn btn-xs btn-primary me-5">
                                                            {{ trans('messages.users.fields.create') }}
                                                        </a>
                                                    @endif
                                                @endif
                            ...
                            @elseif (Request::segment(1) == 'report' && Request::segment(2) == 'stockiest-report-show')
                                <div class="card-toolbar">
                                    <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                        <a href="{{ route('report.stockiest') }}"
                                            class="btn btn-xs btn-primary">Back</a>
                                    </div>
                                </div>
